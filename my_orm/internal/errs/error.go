...
var (
	...
	// 所以放在我们的 internal 包里
	ErrPointerOnly            = errors.New("my_orm: 只支持一级指针作为输入，例如 *User")
	ErrNoRows                 = errors.New("my_orm: 未找到数据")
	ErrTooManyReturnedColumns = errors.New("my_orm: 过多列")
	// ErrInsertZeroRow 代表插入 0 行
	ErrInsertZeroRow    = errors.New("my_orm: 插入 0 行")
	ErrNoUpdatedColumns = errors.New("my_orm: 未指定更新的列")
)
...
func NewErrUnknownField(fd string) error {
	return fmt.Errorf("my_orm: 未知字段 %s", fd)
}
...
func NewErrUnsupportedSelectable(exp any) error {
	return fmt.Errorf("my_orm: 不支持的目标列 %v", exp)
}

// 后面可以考虑支持错误码
// func NewErrUnsupportedExpressionType(exp any) error {
// 	return fmt.Errorf("my_orm-50001: 不支持的表达式 %v", exp)
// }
...
func NewErrInvalidTagContent(tag string) error {
	return fmt.Errorf("my_orm: 错误的标签设置: %s", tag)
}

func NewErrFailToRollbackTx(bizErr error, rbErr error, panicked bool) error {
	return fmt.Errorf("my_orm: 回滚事务失败, 业务错误 %w, 回滚错误 %s, panic: %t",
