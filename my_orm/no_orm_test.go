...
// memoryDB 返回一个基于内存的 my_orm，它使用的是 sqlite3 内存模式。
func memoryDB(t *testing.T, opts ...DBOption) *DB {
	my_orm, err := Open("sqlite3", "file:test.db?cache=shared&mode=memory", opts...)
	if err != nil {
		t.Fatal(err)
	}
	return my_orm
...
func memoryDBWithDB(db string, t *testing.T) *DB {
	my_orm, err := Open("sqlite3", fmt.Sprintf("file:%s.db?cache=shared&mode=memory", db))
	if err != nil {
		t.Fatal(err)
	}
	return my_orm
